...
exp_temp <- exp_temp |> 
  mutate(drop_fund = ifelse(in_ff == 0, "Excluded Fund", "Included Fund"))
```
...
exp_temp <- exp_temp %>% 
  mutate(
    drop_reason = ifelse(in_ff == 0, "Excluded Fund", NA),
    drop_reason = ifelse(
...
```{r create-dropreason}
rev_temp <- rev_temp |> 
  mutate(drop_fund = ifelse(in_ff == "0", 
                               "Excluded Fund", "Included Fund"),
         drop_reason = case_when(
         agency == 799 ~ "Transfer", 
         in_ff == "0" ~ "Excluded Fund",
         TRUE ~ NA))
