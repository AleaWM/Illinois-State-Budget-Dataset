...
# anti_join(exp_allyears, funds_ab_in, by = "fund") # |> distinct(fund)


exp_temp <- exp_allyears %>% 
  filter(!is.na(fy) & !is.na(fund)) |>
  select(-fund_name) %>%
  arrange(fund, fy)  %>%
...
# Object == 1993 is for Interfund cash transfers  
 exp_temp <- exp_temp %>% 
  mutate(transfer = ifelse(org_name == "TRANSFERS", T, F),
         transfer = if_else(agency == 799 & is.na(transfer), T, transfer),
         trans_agency = ifelse(org_name == "TRANSFERS" | agency == 799,
                               str_sub(obj_seq_type,1,3), NA),
         trans_type = ifelse(org_name == "TRANSFERS",
                             str_sub(obj_seq_type, 4,9), NA)) %>%

  mutate(
         object = ifelse(transfer == F, as.character(str_sub(obj_seq_type, 1, 4)), NA_character_),  # appropriation object
         sequence = ifelse(transfer == F, as.character(str_sub(obj_seq_type, 5,6)), NA_character_),
         type = ifelse(transfer == F, str_sub(obj_seq_type, 7,8), NA_character_)    # appropriation type
         )
...
anti_join(rev_allyears, funds_ab_in, by = "fund") # |> distinct(fund)
